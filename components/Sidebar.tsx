  ...
  return (
    <div className="flex flex-col items-center py-4 gap-2 w-full h-full">
      ...
      {NAV.map((item) => {
        ...
        return (
          <button key={item.id} title={item.title} onClick={() => setTab(item.id)}
      ...
      <button title="Import" onClick={() => onOpen('import')}
        ...
        onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.color='var(--text)'; el.style.borderColor='var(--border)'; el.style.background='var(--surface2)'; }}
        onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.color='var(--subtext)'; el.style.borderColor='transparent'; el.style.background='transparent'; }}
      >

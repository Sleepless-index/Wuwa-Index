  ...
  return (
    <div className="tracker-card-wrap flex flex-col" style={{ width: 128 }}>
      <div onClick={handleToggle}
        ...
        style={{ height: 196, border: `1px solid ${owned ? 'rgba(76,123,214,0.6)' : 'rgba(42,48,62,1)'}`, boxShadow: owned ? '0 0 10px rgba(76,123,214,0.08)' : 'none' }}
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={`weapons/${file}.avif`} alt={name}
        ...
        <div className="absolute bottom-0 left-0 right-0 px-2 pb-2 z-10">
          ...
          <p className="text-[10px] font-semibold leading-tight truncate"
            style={{ color: owned ? '#e8e3f0' : 'var(--subtext)' }}>{name}</p>
        </div>
      </div>
      ...
      {owned && (
        <div className="mt-1 px-0.5">
          <button ref={btnRef} onClick={openPanel}
            ...
            style={{ background: isMaxR ? 'rgba(8,12,24,0.92)' : 'rgba(76,123,214,0.12)', borderColor: isMaxR ? 'rgba(76,123,214,0.65)' : 'rgba(76,123,214,0.35)', color: '#4c7bd6' }}
          >R{rank}</button>

          {rankOpen && (
            <div ref={panelRef}
              className="fixed z-[200] flex items-center gap-1 px-2 py-1.5 rounded-lg"
              style={{ top: panelPos.top, left: panelPos.left, background: 'var(--surface2)', border: '1px solid rgba(76,123,214,0.45)', boxShadow: '0 4px 24px rgba(0,0,0,0.4)', whiteSpace: 'nowrap' }}
            >
              ...
              {[1,2,3,4,5].map(i => (
                <button key={i} onClick={() => handleRank(i)}
                  ...
                  style={{ background: rank===i ? 'rgba(76,123,214,0.2)' : 'transparent', color: rank===i ? '#4c7bd6' : 'var(--subtext)', borderColor: rank===i ? 'rgba(76,123,214,0.5)' : 'transparent' }}
                >{i}</button>
              ))}
            </div>
          )}
